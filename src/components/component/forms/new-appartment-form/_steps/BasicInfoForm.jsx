...
const selectedCityId = typeof apartment.city === 'object' ? apartment.city?.id : apartment.city;
const selectedCity = cities.find((c) => c.id == selectedCityId);

  const districtOptions = (selectedCity?.districts ?? []).map(normalizeOption);
  const metroOptions = (selectedCity?.metro_stations ?? []).map(normalizeOption);

  const handleChange = (field) => (e) => {
    const value =
      ['rooms', 'district', 'metro_station', 'price'].includes(field)
        ? +e.target.value
        : e.target.value;
      ...
      {districtOptions.length > 0 && (
        <div>
          ...
          <select
            id="district"
            className="w-full p-2 rounded-xl border px-4 py-2 text-textColor-dark bg-background-default focus:ring-2 focus:ring-primary-default focus:outline-none transition-all duration-300 ease-in-out"
            value={isEditingApartment ? apartment.district?.id : apartment.district || ''}
